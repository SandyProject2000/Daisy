...
class Order(db.Model):
    ...
    payment_slip = db.Column(db.String(255), nullable=True)  # Add this
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    items = db.relationship('OrderItem', backref='order', lazy=True, cascade="all, delete-orphan")

class OrderItem(db.Model):  # <--- Make sure it is aligned to the left
...
@app.route('/upload-slip/<int:order_id>', methods=['GET', 'POST'])
def upload_slip(order_id):
    ...
    if request.method == 'POST':
        ...
        if file and file.filename != '':
            ...
            db.session.commit()
            return redirect(url_for('order_success', code=order.tracking_code))
    return render_template('upload_slip.html', order=order)
...
def checkout():
    ...
    if payment_method == 'Prepaid':
        return redirect(url_for('upload_slip', order_id=new_order.id))
